...
router.put('/profile/saveItinerary/:itinerary_id', (req, res) => {
    ...
    User
        .findByIdAndUpdate(req.body.userId, { $push: { itinerariesSaved: req.params.itinerary_id } }, { new: true })
        .then(response => {
            res.json(response)
        })
        .catch(err => res.status(500).json(err))
...
router.put('/profile/removeItinerary/:itinerary_id', (req, res) => {
    ...
    User
        .findByIdAndUpdate(req.body.userId, { $pull: { itinerariesSaved: req.params.itinerary_id } }, { new: true })
        .then(response => res.json(response))
        .catch(err => res.status(500).json(err))
})
